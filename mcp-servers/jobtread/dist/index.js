...
// ─── Read Tools ─────────────────────────────────────────────────────
server.tool('jobtread_getJob', 'Get the Case Home project details from JobTread (status, description, custom fields)', {}, async () => {
...
server.tool('jobtread_getCostItems', 'Get budget/cost items from JobTread with cost codes and pricing', { limit: z.number().optional().describe('Max items to return (default 100, max 100)') }, async ({ limit }) => {
...
server.tool('jobtread_getTasks', 'Get tasks/schedule from the Case Home job in JobTread', {}, async () => {
    const result = await pave({
        job: {
            $: { id: JOB_ID },
            tasks: {
...
server.tool('jobtread_getDailyLogs', 'Get daily log entries from the Case Home job in JobTread', {}, async () => {
    const result = await pave({
        job: {
            $: { id: JOB_ID },
            dailyLogs: {
...
server.tool('jobtread_getComments', 'Get comments from the Case Home job in JobTread', {}, async () => {
    const result = await pave({
        job: {
            $: { id: JOB_ID },
            comments: {
...
server.tool('jobtread_getFiles', 'Get files (photos, documents) from the Case Home job in JobTread', {}, async () => {
    const result = await pave({
        job: {
            $: { id: JOB_ID },
            files: {
...
server.tool('jobtread_createDailyLog', 'Add a daily log entry to the Case Home job in JobTread', {
...
server.tool('jobtread_createTask', 'Create a task in the Case Home job in JobTread', {
